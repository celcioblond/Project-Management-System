// src/components/AdminDashboard/EditProjectModal.tsx
...
// ─── Shared sub-components (identical pattern) ────────────────────────────────
...
function toDateInput(iso: string): string {
  if (!iso) return '';
  return iso.split('T')[0]; // "2025-01-15T00:00:00" → "2025-01-15"
}
...
export default function EditProjectModal({
  ...
  useEffect(() => {
    if (open && project) {
      ...
      // Match assignedEmployeeNames back to IDs using the employees list
      const preSelected = employees
        .filter((e) => project.assignedEmployeeNames?.includes(e.name))
        .map((e) => e.id);
  ...
  const handleSubmit = async () => {
    if (!project || !validate() || adminId === null) return;
