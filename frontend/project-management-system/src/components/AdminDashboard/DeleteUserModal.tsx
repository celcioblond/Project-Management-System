// src/components/AdminDashboard/DeleteUserModal.tsx
import { useEffect, useState } from 'react';
import { X, TriangleAlert } from 'lucide-react';
import { sileo } from 'sileo';
import { apiService, type UserResponse } from '../../services/api';
...
export default function DeleteUserModal({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      sileo.success({
        title: 'User deleted',
        description: `${user.name || user.username} has been removed.`,
      });
      onDeleted();
      onClose();
    } catch (e) {
      sileo.error({ title: 'Failed to delete user', description: String(e) });
    } finally {
      setSubmitting(false);
    }
  };
