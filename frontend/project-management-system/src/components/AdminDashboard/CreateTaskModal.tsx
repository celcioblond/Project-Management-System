// src/components/AdminDashboard/CreateTaskModal.tsx
...
export default function CreateTaskModal({
  ...
  // Only show employees assigned to the selected project (if one is chosen),
  ...
  const eligibleEmployees = selectedProject
    ? employees.filter(
        (e) =>
          e.role === 'EMPLOYEE' &&
          selectedProject.assignedEmployeeNames?.includes(e.name),
      )
    : employees.filter((e) => e.role === 'EMPLOYEE');

  const filteredEmployees = eligibleEmployees.filter(
    (e) =>
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await apiService.createTask(payload, adminId);
      sileo.success({
        title: 'Task created',
      ...
      sileo.error({ title: 'Failed to create task', description: String(e) });
    } finally {
      setSubmitting(false);
    }
  };
              ...
              <div className="max-h-40 overflow-y-auto divide-y divide-slate-100">
                {filteredEmployees.length === 0 ? (
                  <p className="text-xs text-slate-400 px-3 py-3 text-center">
                    {form.projectId && eligibleEmployees.length === 0
